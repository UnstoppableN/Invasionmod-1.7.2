...
//needed?
			...
			if ((mod_Invasion.tryGetInvasionPermission(this)) && (this.nexusItemStacks[0] != null)) {
				if (this.nexusItemStacks[0].getItem() == mod_Invasion.itemNexusCatalyst) {
					...
					if (this.nexusItemStacks[0].stackSize == 0)
						this.nexusItemStacks[0] = null;
					startInvasion(1);
					//this.worldObj.getBlock(this.xCoord, this.yCoord, this.zCoord).setBlockUnbreakable();
	...
	private void stop() {
		if (this.mode == 3) {
			...
			setMode(0);
			//this.worldObj.getBlock(this.xCoord, this.yCoord, this.zCoord).setHardness(3.0F);
	...
	private float getPlayerRelativeBlockHardness(EntityPlayer player, World world, int x, int y, int z){
		return z;
	...
	private void playSoundForBoundPlayers(String sound)
	{
		...
		if (boundPlayers != null) {
	        ...
	        {
				EntityPlayerMP player=(EntityPlayerMP)FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager().getPlayerForUsername((String) entry.getKey());
				player.getEntityWorld().playSoundAtEntity((Entity)player,sound, 1.0F, 1.0F);
	        }
		}
	}
